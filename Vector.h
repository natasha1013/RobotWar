...
public:
    ...
    iterator insert(const_iterator pos, const T& val) noexcept{
        size_t idx = pos - storage;
        if (vector_size == reserved_size){
            reserved_size += reserved_size / 2 + 1;
            reallocate();
        }
        move_storage(storage + idx + 1, storage + idx, vector_size - idx);
        storage[idx] = val;
        vector_size++;
        return storage + idx;
    }

    iterator insert(const_iterator pos, T&& rval) noexcept{
        size_t idx = pos - storage;
        if (vector_size == reserved_size) {
            reserved_size += reserved_size / 2 + 1;
            reallocate();
        ...
        move_storage(storage + idx + 1, storage + idx, vector_size - idx);
        storage[idx] = std::move(rval);
        vector_size++;
        return storage + idx;
    }

    iterator insert(const_iterator pos, size_t n, const T& val) noexcept{
        size_t idx = pos - storage;
        if (vector_size + n > reserved_size) {
            reserved_size = vector_size + n + (vector_size + n) / 2 + 1;
            reallocate();
        }
        move_storage(storage + idx + n, storage + idx, vector_size - idx);
